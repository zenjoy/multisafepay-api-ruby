    ...
    class << self
      ...
      def all(options = {})
        ...
        request('GET', id, data, options) do |response|
          MultiSafePay::List.new(response, self)
        end
      end

      def get(id, options = {})
        raise MultiSafePay::Exception, "Invalid resource ID: #{id.inspect}" if id.nil? || id.strip.empty?
      ...
      def request(method, id = 0, data = {}, options = {})
        ...
        response  = MultiSafePay::Client.instance.perform_http_call(method, resource_name(parent_id), id, data, options)
